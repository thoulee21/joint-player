...
export const DataItem = ({ item }: { item: DataItemType }) => {
  ...
  const Content = useCallback(() => {
    ...
    switch (typeof item.data) {
      ...
      case "boolean":
        content = <Text style={styles.dataText}>{item.data.toString()}</Text>;
        break;

      case "undefined":
        content = <Text style={styles.dataText}>empty</Text>;
        break;
  ...
  const renderDataAvatar = useCallback(
    (props: any) => (
      <Avatar.Text {...props} label={item.name[0].toLocaleUpperCase()} />
    ),
    [item.name],
  );

  const renderMoreButton = useCallback(
    (props: { size: number }) => <DataMoreBtn data={item.data} props={props} />,
    [item],
  );
  ...
  return (
    <Card mode="contained" style={styles.card}>
      <Card.Title
        title={item.name}
        subtitle={!isArray ? typeof item.data : `array[${item.data.length}]`}
