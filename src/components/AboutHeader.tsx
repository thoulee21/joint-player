...
export const AboutHeaderComponent = ({ showNavBar }: ScrollHeaderProps) => {
  const appTheme = useTheme();
  const navigation = useNavigation();
  const { top } = useSafeAreaInsets();

  const renderBackground = useCallback(
    ({ showNavBar: opacity }: SurfaceComponentProps) => (
      <FadingView
        opacity={opacity}
        style={[
  ...
  return (
    <Header
      ...
      headerStyle={{
        height: 64 + top,
        backgroundColor: appTheme.dark ? appTheme.colors.surface : THEME_COLOR,
      }}
    ...
    >
      <ScalingView scrollY={scrollY}>
        <View>
          ...
          />
          <Text variant="titleSmall" style={[styles.name, styles.white]}>
            {packageData.displayName}
          </Text>
        </View>
      </ScalingView>
