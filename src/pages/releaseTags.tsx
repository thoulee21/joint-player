...
export const ReleaseTags = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR<Main[]>(
    `https://api.github.com/repos/${userRepo}/tags`,
  );
  ...
  const renderHeaderRight = useCallback(
    (props: any) => (
      <IconButton {...props} icon="refresh" onPress={() => mutate()} />
    ),
    [mutate],
  );
  ...
  const renderReleaseTagIcon = useCallback(
    (props: ListLRProps) => <List.Icon {...props} icon="tag-outline" />,
    [],
  );
    ...
    ({ item }: { item: Main }) => {
      return (
        <List.Item
          ...
          onPress={() => {
            //@ts-expect-error
            navigation.push("ChangeLog", { version: item.name });
          }}
          onLongPress={() => {
